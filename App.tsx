...
const App: React.FC = () => {
  ...
  const [neuralMemory, setNeuralMemory] = useState<string>('Initializing latent link...');
  ...
  useEffect(() => {
    const activeModes = [AppMode.PORTRAIT_GENERATOR, AppMode.GROUP_PHOTO, AppMode.ACCESSORIES_GENERATOR, AppMode.THUMBNAIL_CREATOR];
    if (activeModes.includes(state.currentMode)) {
      if (state.promptHistory.length > 0) {
        extractNeuralMemory(state.promptHistory).then(m => setNeuralMemory(m));
      }
    }
  }, [state.promptHistory, state.currentMode]);

  const handleClearMemory = () => {
    setNeuralMemory('Buffer purged.');
    ...
    try {
      ...
      if (response.images?.length > 0) {
        ...
        setState(prev => ({ 
          ...
          generatedGallery: [{ url: newImg, prompt: finalPrompt, settings: state.settings, timestamp: Date.now() }, ...prev.generatedGallery]
        }));
      }
    } catch (error: any) { 
  ...
  const isEditorMode = [AppMode.PORTRAIT_GENERATOR, AppMode.GROUP_PHOTO, AppMode.ACCESSORIES_GENERATOR, AppMode.THUMBNAIL_CREATOR].includes(state.currentMode);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#05080f] text-slate-100 overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-8">
          {isEditorMode && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-900/50 rounded-lg border border-white/5">
               <Radio size={14} className={neuralMemory.length > 30 ? "text-red-500 animate-pulse" : "text-slate-600"} />
               <span className={`text-[9px] font-black uppercase tracking-widest ${neuralMemory.length > 30 ? "text-slate-200" : "text-slate-600"}`}>Live Learning</span>
            </div>
          )}
          {isEditorMode && (
            <button onClick={() => setShowMemoryLog(true)} className={`flex items-center gap-2 px-4 py-2 border rounded-full transition-all group ${neuralMemory.length > 30 ? 'bg-yellow-500/10 border-yellow-500/20' : 'bg-white/5 border-white/10'}`}>
                <BrainCircuit size={14} className={neuralMemory.length > 30 ? "text-yellow-500" : "text-slate-500"} />
                <span className={`text-[10px] font-black uppercase tracking-widest ${neuralMemory.length > 30 ? "text-yellow-500" : "text-slate-500"}`}>Neural Active</span>
            </button>
          )}
      ...
      <main className="flex-1 relative">
        {state.currentMode === AppMode.LANDING && <LandingPage onSelectMode={(m) => (m === AppMode.GROUP_PHOTO || m === AppMode.PORTRAIT_GENERATOR) ? (setPendingMode(m), setShowSeedModal(true)) : setState(prev => ({ ...prev, currentMode: m }))} />}
        {isEditorMode && (
          <EditorWorkspace 
            mode={state.currentMode} settings={state.settings} negativePrompt={state.negativePrompt} prompt={prompt}
            canUndo={historyIndex > 0} canRedo={historyIndex < history.length - 1} onUndo={handleUndo} onRedo={handleRedo}
            onUpdatePrompt={(p) => setPrompt(p)} onCommitPrompt={(p) => pushToHistory(p, state.settings, state.negativePrompt)}
            neuralMemory={neuralMemory} onUpdateSettings={(s) => { setState(prev => ({ ...prev, settings: s })); pushToHistory(prompt, s, state.negativePrompt); }}
            onUpdateNegativePrompt={(np) => { setState(prev => ({ ...prev, negativePrompt: np })); pushToHistory(prompt, state.settings, np); }}
            onGenerate={handleGenerate} isLoading={isLoading} previewImage={lastGeneratedImage}
            onSelectFromGallery={(img) => setLastGeneratedImage(img)} gallery={state.generatedGallery} seeds={state.seeds}
            ...
            autoFillImage={autoFillImage} onAutoFillConsumed={() => setAutoFillImage(null)}
        ...
        <div className="fixed inset-0 z-[5000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
           <div className="w-full max-w-2xl bg-[#0a0f1d] border border-yellow-500/30 rounded-[2rem] overflow-hidden flex flex-col shadow-2xl">
              <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-yellow-500/5">
                 <div className="flex items-center gap-3"><Terminal size={18} className="text-yellow-500" /><h3 className="text-sm font-black text-white uppercase tracking-widest">Neural Memory Core</h3></div>
                 <div className="flex items-center gap-2">
                    <button onClick={handleClearMemory} className="p-2 hover:bg-red-500/10 rounded-lg text-red-500 transition-all flex items-center gap-2 text-[9px] font-black uppercase"><Trash2 size={14} /> Purge</button>
              ...
              <div className="p-8 font-mono text-xs text-slate-300 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                 <div className="p-4 bg-black/40 rounded-xl border border-white/5">
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2"><Database size={12} /> Active Extraction</div>
                    {neuralMemory.length > 5 ? <div className="whitespace-pre-wrap text-yellow-500/90 font-medium leading-relaxed">{neuralMemory}</div> : <div className="text-slate-600 italic">Capturing biometric patterns...</div>}
                 </div>
              </div>
           </div>
        </div>
